...
def recordRank(request):
    month = request.GET.get('month')
    if month =="0" or month == None:
        record = Record.objects.values('user__username').annotate(record_sum=Sum('distance')).order_by('-record_sum')
    else:
        record = Record.objects.filter(record_date__month=month).values('user__username').annotate(record_sum=Sum('distance')).order_by('-record_sum')
